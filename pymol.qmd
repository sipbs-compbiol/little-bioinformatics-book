...
For example, to select only the first chain of the trimeric PduB structure (chain A), we would execute the command `indicate chain A`, which highlights that chain in the main visualisation window (@fig-pymol_chain_highlight).
